...
시간 복잡도 함수 T(n)에서 가장 영향력이 큰 부분을 따지는 방법으로 빅-오 표기법(Big-Oh Notation)을 사용. 빅-오의 수학적 정의: 두 개의 함수 $f(n)$과 $g(n)$이 주어졌을 때, 모든 $n\geqK$에 대하여 $f(n)\leqCg(n)$을 만족하는 두 개의 상수 $C$와 $K$가 존재하면, $f(n)$의 빅-오는 $O(g(n))$($n\geq0$, $f(n)\geq0$, $g(n)\geq0$. 즉, 빅-오는 증가율의 상한선을 표현하는 표기법. <br>
